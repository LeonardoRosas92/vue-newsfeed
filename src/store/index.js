...
export default createStore({
  ...
  actions: {
    ...
    UpdatePost({ commit }, post){
      commit('SET_POST', this.state.posts.map(p => p._id === post._id ? post : p))
    },
    AddNewPost({ commit }, post){
      commit('SET_POST', [...this.state.posts, post])
      commit('INCREMENT_TOTAL_POSTS')
    },
    RemovePost({commit}, id){
      commit('SET_POST', this.state.posts.filter( p => p._id !== id));
    ...
    LoadMorePosts({commit}, limit = 10){
      ...
      sanity.fetch(query).then(posts => {
        commit('SET_POST', [...this.state.posts, ...posts])
      })
    },
    FetchAuthors({ commit }) {
